  ...
  return (
    <div className="space-y-2">
      {chapters.map((chapter, index) => {
        ...
        // Bloqueado si el capitulo anterior no esta completado
        const isLocked = index > 0 && !chapters[index - 1].progress.completed
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {chapter.is_free && !isLocked && (
                  <span className="flex-shrink-0 text-xs px-2 py-0.5 rounded bg-green-500/20 text-green-400">
                    Gratis
                  </span>
                )}
                {isLocked && (
                  <span className="flex-shrink-0 text-xs px-2 py-0.5 rounded bg-slate-700/50 text-slate-500">
                    Bloqueado
                  </span>
                )}
              </div>
        ...
        // Si esta bloqueado, renderizar div sin link
        if (isLocked) {
          return (
            <div
              key={chapter.id}
              className="block p-4 rounded-xl border bg-slate-800/20 border-slate-700/30 cursor-not-allowed opacity-70"
            >
              {content}
            </div>
          )
        ...
        // Si no esta bloqueado, renderizar Link
        return (
          <Link
            key={chapter.id}
